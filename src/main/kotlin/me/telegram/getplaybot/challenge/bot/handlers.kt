...
fun handlers(bot: ChallengeHandlers): BotHandlers {
    val list = listOf(
        object : BotHandle("start") {
            suspend override fun execute(message: Message, user: User, payload: String) {
                ...
                bot.markdown(message) { handleWelcome(bot.handlers, user, message.from) }
